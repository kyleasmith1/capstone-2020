...
public final class RecommenderAlgorithmTest {
    ...
    @Test
    public void populateUserTagMapNoTagsOneRoomTest() { 
        TEST_ROOM_RUN.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_RUN.getRoomEntity());

        RecommenderAlgorithm.populateEmbeddedEntity(TEST_USER_SMITH);
        EmbeddedEntity userTagMap = TEST_USER_SMITH.getEmbeddedTags();
        for(String tag : TEST_CATEGORY_TAGS){
            if(tag.equals(Tag.TAG_FITNESS)){
                Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
                continue;
            }
            Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
        }
    ...
    public void populateUserTagMapOneTagOneRoomTest() { 
        ...
        for(String tag : TEST_CATEGORY_TAGS){
            if(tag.equals(Tag.TAG_FITNESS)){
                Assert.assertEquals((double) userTagMap.getProperty(tag), 1.0, epsilon);
                continue;
            }
            Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
        }
    ...
    @Test
    public void populateUserTagMapMultipleTagMultipleRoomsTest() { 
        TEST_ROOM_POETRY.addTag(Tag.TAG_EDUCATION);
        TEST_ROOM_POETRY.addTag(Tag.TAG_LITERATURE);
        TEST_ROOM_POETRY.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_POETRY.getRoomEntity());
    ...
    @Test
    public void populateUserTagMapOneUserSameTagMultipleRoomsTest() { 
        ...
        for(String tag : TEST_CATEGORY_TAGS) {
            if(tag.equals(Tag.TAG_COOKING)) {
                Assert.assertEquals((double) userTagMap.getProperty(tag), 2.0, epsilon);
                continue;
            }
            Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
        }
    ...
    @Test
    public void normalizeUserTagMapNoTagsOneRoomTest() { 
        TEST_ROOM_RUN.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_RUN.getRoomEntity());
    ...
    @Test
    public void normalizeUserTagMapOneTagOneRoomTest() { 
        TEST_ROOM_RUN.addTag(Tag.TAG_FITNESS);
        TEST_ROOM_RUN.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_RUN.getRoomEntity());
    ...
    @Test
    public void normalizeUserTagMapMultipleTagOneRoomTest() { 
        TEST_ROOM_POETRY.addTag(Tag.TAG_EDUCATION);
        TEST_ROOM_POETRY.addTag(Tag.TAG_LITERATURE);
        TEST_ROOM_POETRY.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_POETRY.getRoomEntity());
        ...
        for(String tag : TEST_CATEGORY_TAGS) {
            if(tag.equals(Tag.TAG_EDUCATION) || tag.equals(Tag.TAG_LITERATURE)) {
                Assert.assertEquals((double) userTagMap.getProperty(tag), 0.7071067811865475, epsilon);
                continue;
            }
            Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
        }
    ...
    @Test
    public void normalizeUserTagMapMultipleTagMultipleRoomsTest() { 
        TEST_ROOM_POETRY.addTag(Tag.TAG_EDUCATION);
        TEST_ROOM_POETRY.addTag(Tag.TAG_LITERATURE);
        TEST_ROOM_POETRY.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_POETRY.getRoomEntity());
        ...
        for(String tag : TEST_CATEGORY_TAGS) {
            if(tag.equals(Tag.TAG_FITNESS) || tag.equals(Tag.TAG_EDUCATION) 
                || tag.equals(Tag.TAG_LITERATURE)) {
                Assert.assertEquals((double) userTagMap.getProperty(tag), 0.5773502691896258, epsilon);
                continue;
            }
            Assert.assertEquals((double) userTagMap.getProperty(tag), 0.0, epsilon);
        }
    ...
    @Test
    public void normalizeUserTagMapSameTagMultipleRoomsTest() { 
        TEST_ROOM_COOK.addTag(Tag.TAG_COOKING);
        TEST_ROOM_COOK.addFollower(TEST_USER_SMITH);
        DatabaseService.save(TEST_ROOM_COOK.getRoomEntity());
